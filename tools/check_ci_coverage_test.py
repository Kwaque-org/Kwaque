...
WORKFLOW_ARGUMENTS = len(sys.argv) == 4 and sys.argv[1].endswith((".yml", ".yaml"))
WORKFLOW = (
    Path(sys.argv[1])
...
FUZZ_WORKFLOW = Path(sys.argv[2]) if WORKFLOW_ARGUMENTS else WORKFLOW.with_name("fuzz.yml")
BAZEL_CONFIG = Path(sys.argv[3]) if WORKFLOW_ARGUMENTS else WORKFLOW.parents[2] / ".bazelrc"
STATEFUL_FUZZERS = {"scheduler_fuzz", "fault_schedule_fuzz", "fake_file_fuzz", "fake_network_fuzz"}
SMOKE_FUZZERS = {f"//src/simulation/tests:{name}" for name in STATEFUL_FUZZERS} | {
    "//src/config:bootstrap_config_fuzz", "//proto/kwaque/common/v1:build_info_fuzz",
    "//src/bytes:fragmented_buffer_fuzz", "//src/simulation/tests:signal_canary_test",
}
...
def fuzz_coverage_errors(workflow: str, scheduled: str, config: str) -> list[str]:
    ...
    if len(runs) != 1:
        ...
        if targets != SMOKE_FUZZERS:
            errors.append("smoke must execute every fuzzer and the verifying signal canary")
        for flag in ("--config=ci", "--config=fuzz", "--keep_going", "--test_output=all",
                     "--test_arg=-max_total_time=2", "--test_arg=-seed=1"):
    ...
    for option in ("--copt", "--linkopt"):
        if f"build:san-all {option}=-fsanitize=address,undefined,vptr,function,alignment" not in config.splitlines():
            errors.append("fuzz mode must compile and link address and undefined behavior checks")
    ...
    if targets != STATEFUL_FUZZERS:
        errors.append("scheduled matrix must contain every stateful target exactly once")
    ...
    if len(commands) != 1 or not commands[0].startswith("bazel test "):
        ...
        for flag in ("--config=ci", "--config=fuzz", "--test_output=all", "--test_timeout=720",
                     "--test_arg=-max_total_time=600", '"//src/simulation/tests:${FUZZ_TARGET}"'):
    ...
    for name, job in (("smoke", smoke), ("scheduled", campaign)):
        for required in ("--test_env=KWAQUE_FUZZ_MINIMIZE_SECONDS=30", "if: always()",
                         "actions/upload-artifact@", "bazel-testlogs/**/test.log",
                         "bazel-testlogs/**/test.outputs/**", "retention-days: 30"):
            if required not in job:
                errors.append(f"{name} must retain bounded failure diagnostics: {required}")
    repeated = run_commands(job_blocks(workflow).get("test", ""))
    if not any(all(flag in command.split() for flag in (
        "--runs_per_test=10", "--cache_test_results=no", "--spawn_strategy=sandboxed",
        "//src/runtime/tests:hermetic_contracts")) for command in repeated):
...
def analysis_coverage_errors(workflow: str) -> list[str]:
    ...
    if not any("bazel build --config=ci-debug --build_tag_filters=-fuzz,-manual //..." == command for command in ordinary):
        errors.append("ordinary analysis must materialize all selected translation units")
    for target in ("//tools:compile_commands", "//tools:clang_tidy", "//tools:clang_tidy_strict"):
    ...
    if not any("--fuzz-only" in command.split() and "//tools:compile_commands" in command.split() for command in fuzz):
        errors.append("fuzz analysis needs its own compilation database")
    if not any("--build_tag_filters=fuzz" in command.split() and "//..." in command.split() for command in fuzz):
...
def job_blocks(workflow: str) -> dict[str, str]:
    ...
    return {
        match.group(1): workflow[match.end():starts[index + 1].start() if index + 1 < len(starts) else len(workflow)]
...
def coverage_errors(workflow: str) -> list[str]:
    ...
    for name, action, config in (
        ...
        broad = [command for command in commands
                 if re.search(rf"\bbazel {action}\b", command)
                 and f"--config={config}" in command and "//..." in command.split()]
        if len(broad) != 1:
            errors.append(f"{name}: requires one broad {config} {action} command")
            continue
        command = broad[0]
        if "--build_tag_filters=-fuzz,-manual" not in command:
            errors.append(f"{name}: build selection must exclude fuzz and manual targets")
        if action == "test" and "--test_tag_filters=-fuzz,-manual" not in command:
            errors.append(f"{name}: test execution must exclude fuzz and manual targets")
        if "--build_tests_only" in command or "-benchmark" in command:
            errors.append(f"{name}: ordinary builds must include benchmark and other translation units")
    ...
    for field, expected in (
        ...
        if values != expected:
            errors.append(f"goldens: requires both {field} values for all four native jobs")
    commands = run_commands(goldens)
    expected = f'bazel test --config="${{{{ matrix.config }}}}" --cache_test_results=no {GOLDENS}'
    if commands != [expected]:
        errors.append("goldens: all four jobs must execute the identical explicit uncached suite")
...
class CiCoverageTest(unittest.TestCase):
    ...
    def test_analysis_covers_ordinary_and_fuzz_sources(self) -> None:
        ...
        for token in ("--fuzz-only", "--build_tag_filters=fuzz", "//tools:clang_tidy_strict"):
            with self.subTest(token=token):
                self.assertTrue(analysis_coverage_errors(self.workflow.replace(token, "")))
        job = job_blocks(self.workflow)["clang-tidy-fuzz"]
        changed = self.workflow.replace(job, job.replace("//tools:clang_tidy", "//tools:clang_tidy_strict"))
        self.assertTrue(analysis_coverage_errors(changed))

    def test_fuzz_and_hermetic_coverage(self) -> None:
        self.assertEqual(fuzz_coverage_errors(self.workflow, FUZZ_WORKFLOW.read_text(), BAZEL_CONFIG.read_text()), [])

    def test_each_omitted_smoke_target_or_missing_cap_fails(self) -> None:
        for value in SMOKE_FUZZERS | {"--test_arg=-max_total_time=2", "--test_output=all", "--runs_per_test=10"}:
            with self.subTest(value=value):
                self.assertTrue(fuzz_coverage_errors(self.workflow.replace(value, ""),
                                                    FUZZ_WORKFLOW.read_text(), BAZEL_CONFIG.read_text()))

    def test_each_omitted_campaign_or_retention_setting_fails(self) -> None:
        scheduled = FUZZ_WORKFLOW.read_text()
        for value in STATEFUL_FUZZERS | {"--test_arg=-max_total_time=600", "if: always()",
                                       "--test_timeout=720", "fail-fast: false", "  schedule:"}:
            with self.subTest(value=value):
                self.assertTrue(fuzz_coverage_errors(self.workflow, scheduled.replace(value, ""),
    ...
    def test_dropping_ubsan_or_replacing_tests_with_builds_fails(self) -> None:
        ...
        self.assertTrue(fuzz_coverage_errors(self.workflow, FUZZ_WORKFLOW.read_text(),
                                            config.replace("build:fuzz --config=san-all", "build:fuzz --config=dev-base")))
        self.assertTrue(fuzz_coverage_errors(self.workflow, FUZZ_WORKFLOW.read_text(),
                                            config.replace("address,undefined,vptr,function,alignment", "address")))
        self.assertTrue(fuzz_coverage_errors(self.workflow.replace("bazel test --config=ci --config=fuzz", "bazel build --config=ci --config=fuzz"),
                                            FUZZ_WORKFLOW.read_text(), config))
    ...
    def test_missing_filters_and_benchmark_exclusion_fail(self) -> None:
        for flag in ("--build_tag_filters=-fuzz,-manual", "--test_tag_filters=-fuzz,-manual"):
            with self.subTest(flag=flag):
                self.assertTrue(coverage_errors(self.workflow.replace(flag, "")))
        self.assertTrue(coverage_errors(self.workflow.replace(
            "--build_tag_filters=-fuzz,-manual", "--build_tag_filters=-fuzz,-manual,-benchmark")))
    ...
    def test_each_missing_golden_dimension_or_changed_suite_fails(self) -> None:
        for value in ("ubuntu-24.04", "ubuntu-24.04-arm", "ci-debug", "ci-release"):
            with self.subTest(value=value):
                self.assertTrue(coverage_errors(self.workflow.replace(f"          - {value}\n", "")))
        self.assertTrue(coverage_errors(self.workflow.replace(GOLDENS, "//src/simulation/tests:philox_kat_test")))
        self.assertTrue(coverage_errors(self.workflow.replace("--cache_test_results=no", "")))

    def test_actual_test_execution_and_configuration_isolation_are_required(self) -> None:
        job = job_blocks(self.workflow)["test"]
        self.assertTrue(coverage_errors(self.workflow.replace(job, job.replace("bazel test", "bazel build"))))
        self.assertTrue(coverage_errors(self.workflow.replace(job, job + "\n      - run: bazel build --config=ci-release //:kwaque\n")))
