...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="signal-canary-", dir=parent) as temporary:
        ...
        require(
            Path.cwd() == original_cwd, "wrapper changed the parent working directory"
        )
        ...
        require(
            minimized[0].read_bytes() == b"KQS1",
            "canary was not minimized to its four-byte trigger",
        )
        log = (failures[0].parent / "campaign.log").read_text(errors="replace")
        require(
            "KQFUZZ HARNESS=signal_canary VERSION=1" in log,
            "canary identity was not emitted",
        )
        ...
        replay = wrapper.run_logged(
            [
                ...
                "-runs=1",
                "-timeout=15",
                f"-artifact_prefix={outputs}/",
                str(minimized[0]),
            ],
            root,
            wrapper.normalized_environment(),
            outputs / "replay.log",
            20,
        )
        require(
            replay != 0 and replay != 124,
            "canary artifact did not reproduce the signal",
        )
        require(
            "KQFUZZ HARNESS=signal_canary VERSION=1"
            in (outputs / "replay.log").read_text(errors="replace"),
            "artifact replay did not emit the canary identity",
        )
        require(
            set(original_cwd.glob("crash-*")) == original_crashes,
            "canary polluted the runfiles directory",
        )
