...
class FuzzReproductionTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        temporary = tempfile.TemporaryDirectory(prefix="reproduction-", dir=os.environ["TEST_TMPDIR"])
    ...
    @staticmethod
    def process_output(completed: subprocess.CompletedProcess[str]) -> str:
        return f"stdout:\n{completed.stdout}\nstderr:\n{completed.stderr}"

    def assert_replay_exit(self, block: str, expected: int) -> subprocess.CompletedProcess[str]:
    ...
    def mutate_section(block: str, name: str) -> str:
        ...
        section = lines.index(next(line for line in lines if line.startswith(f"SECTION {name} ")))
        data = next(index for index in range(section + 1, len(lines)) if lines[index].startswith("D "))
    ...
    def change_event_with_valid_integrity(block: str) -> str:
        lines = block.splitlines()
        start = next(index for index, line in enumerate(lines) if line.startswith("SECTION EVENTS "))
        ...
        event_digest = next(index for index, line in enumerate(lines) if line.startswith("EVENT_DIGEST "))
    ...
    def refresh_envelope_integrity(lines: list[str]) -> str:
        metadata = dict(line.split(" ", 1) for line in lines[1:lines.index(next(line for line in lines if line.startswith("SECTION ")))])
        integrity = hashlib.sha256(b"KQREPRO" + (1).to_bytes(4, "big"))
        for key in (
            "HARNESS", "HARNESS_VERSION", "MASTER_SEED", "RANDOM_ALGORITHM",
            ...
            "TRACE_DIGEST", "EVENT_DIGEST",
        ):
            integrity.update(bytes.fromhex(metadata[key]))
        envelope_digest = next(index for index, line in enumerate(lines) if line.startswith("ENVELOPE_DIGEST "))
    ...
    def obsolete_harness_with_valid_integrity(block: str) -> str:
        lines = block.splitlines()
        version = next(index for index, line in enumerate(lines) if line.startswith("HARNESS_VERSION "))
    ...
    def test_semantic_failure_is_complete_host_independent_and_replayable(self) -> None:
        ...
        divergent = self.assert_replay_exit(self.change_event_with_valid_integrity(block), 1)
        ...
        self.assert_replay_exit(self.replace_section_size(block, "INPUT", 16 * 1024 + 1), 2)
        self.assert_replay_exit(self.replace_section_size(block, "TRACE", 128 * 1024 + 1), 2)
