...
template<typename Value>
std::expected<Value, config_error> required_scalar(
  const YAML::Node& mapping, std::string_view key, std::string field) {
...
bootstrap_config_result decode_bootstrap_config(const YAML::Node& root) {
    ...
    if (settings["node_id"]) {
        ...
        if (
          *node_id < 0 || *node_id > std::numeric_limits<std::int32_t>::max()) {
            return std::unexpected(make_error(
    ...
    if (settings["admin"]) {
        ...
        if (
          auto validated = validate_keys(admin, "kwaque.admin", admin_keys);
          !validated) {
            return std::unexpected(std::move(validated.error()));
        }
            ...
            if (
              ...
              || std::ranges::any_of(*address, [](unsigned char character) {
                ...
                return std::unexpected(make_error(
                  ...
                  "admin address must be non-empty and contain no whitespace "
                  "or "
                  "control characters"));
            }
        ...
        if (admin["port"]) {
            const auto port = required_scalar<std::int64_t>(
