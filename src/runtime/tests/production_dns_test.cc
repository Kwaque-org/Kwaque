...
SEASTAR_TEST_CASE(production_dns_numeric_bypasses_native_resolution) {
    ...
    const auto exercise = [&resolver] -> seastar::future<> {
    ...
    co_await exercise().finally([&resolver] {
        return resolver.stop().then(kwaque::runtime::testing::require_cleanup);
    });
}
...
SEASTAR_TEST_CASE(production_dns_preserves_answer_order_ttl_and_split_input) {
    ...
    const auto exercise = [&] -> seastar::future<> {
        ...
        BOOST_CHECK(
          resolved->answers()[0].endpoint.address().bytes()[3]
          == std::byte{42});
        BOOST_CHECK(
          resolved->answers()[1].endpoint.address().bytes()[3]
          == std::byte{43});
        BOOST_CHECK(
    ...
    co_await exercise()
      .finally([&resolver] {
          return resolver.stop().then(
            kwaque::runtime::testing::require_cleanup);
      })
      .finally([&listener, &serving] {
...
SEASTAR_TEST_CASE(production_dns_stop_drains_active_and_wakes_waiter) {
    ...
    const auto exercise = [&] -> seastar::future<> {
        ...
        auto active = resolver.resolve(
          make_dns_query("active.test"), active_abort);
...
SEASTAR_TEST_CASE(production_dns_maps_server_error_and_result_bound) {
    ...
    }
    {
        ...
        const auto exercise = [&] -> seastar::future<> {
            ...
            BOOST_CHECK(
              bounded.error().code() == kwaque::errc::resource_exhausted);
            const auto stopped = co_await resolver.stop();
            BOOST_REQUIRE(stopped.has_value());
        };
        co_await exercise()
