...
seastar::sstring full_name(kwaque::metric_id id) {
    ...
    auto full_name = seastar::sstring{std::string{descriptor.group}};
    full_name += seastar::sstring{"_"};
    full_name += seastar::sstring{std::string{descriptor.name}};
    return full_name;
}
...
SEASTAR_TEST_CASE(runtime_metric_inventory_registers_and_unregisters_by_owner) {
    ...
    kwaque::runtime::runtime_service runtime{std::ref(parent)};
    co_await runtime.start();
    ...
    const auto accepted = runtime.tasks().spawn(
    ...
    release.set_value();
    while (runtime.tasks().task_count() != 0) {
    ...
    auto backend = kwaque::runtime::production::backend_test_access::make(
      parent);
    co_await backend->start();
    for (std::uint16_t value = static_cast<std::uint16_t>(
