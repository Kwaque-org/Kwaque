...
pid_file::pid_file(std::filesystem::path path) : path_(std::move(path)) {
  descriptor_ = ::open(path_.c_str(), O_RDWR | O_CREAT | O_CLOEXEC | O_NOFOLLOW,
                       0644);
  if (descriptor_ < 0) {
  ...
  try {
    ...
    if (!S_ISREG(status.st_mode)) {
      throw std::runtime_error("PID file is not a regular file: " +
                               path_.string());
    }
...
void pid_file::remove_if_owned() noexcept {
  ...
  if (static_cast<unsigned long long>(descriptor_status.st_dev) != device_ ||
