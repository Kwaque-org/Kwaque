...
seastar::future<result<item_count>> scan_header_extensions_in_transaction(
  ...
  field_context context) {
    ...
    while (remaining != 0) {
        ...
        const auto tag = seastar::read_le<std::uint16_t>(encoded.data());
        const auto flags = seastar::read_le<std::uint16_t>(encoded.data() + 2);
        const auto value_length = seastar::read_le<std::uint32_t>(
          encoded.data() + 4);
