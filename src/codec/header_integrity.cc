...
constexpr std::array<char, 4> zero_checksum{};
...
error integrity_error(
  ...
  std::uint64_t offset) noexcept {
    return error{code, context.family, field, offset};
}

} // namespace

seastar::future<result<void>> verify_header_crc(
  const bytes::fragmented_buffer& header,
  cooperative_work& work,
  field_context context,
  header_crc_layout layout) {
    KWAQUE_INVARIANT(
      invariant_id{"KQ-HEADER-CRC-LAYOUT"},
      layout.prefix_bytes >= 4
        && layout.checksum_offset <= layout.prefix_bytes - 4
        && layout.header_length_offset < layout.prefix_bytes,
      "header checksum layout exceeds its fixed prefix");
    const auto checksum_begin = layout.checksum_offset;
    const auto checksum_end = checksum_begin + 4;
    const auto initial_anchor = integrity_error(
      errc::success, context, layout.checksum_field, context.origin);
    ...
    if (
      ...
      > std::numeric_limits<std::uint64_t>::max() - context.origin) {
        co_return codec::failure(integrity_error(
          ...
          context,
          layout.header_length_field,
          context.origin));
    }
    if (header_size < layout.prefix_bytes) {
        co_return codec::failure(integrity_error(
          errc::malformed_data,
          context,
          layout.header_length_field,
    ...
    if (
      ...
      !bounded) {
        KWAQUE_INVARIANT(
          invariant_id{"KQ-HEADER-CRC-BOUNDS"},
        ...
        co_return codec::failure(integrity_error(
          ...
          context,
          layout.header_length_field,
          context.origin + layout.header_length_offset));
    }
    const auto anchor = integrity_error(
      errc::success,
      context,
      layout.checksum_field,
    ...
    KWAQUE_INVARIANT(
      invariant_id{"KQ-HEADER-CRC-COVERAGE"},
