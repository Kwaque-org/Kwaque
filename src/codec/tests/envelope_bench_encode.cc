...
seastar::future<result<fragmented_buffer>> encode_owned(
  ...
  field_context context) {
    ...
    auto prefix = native_prefix_write(
    ...
    const error body_anchor{
      ...
      context.origin + 24};
    const auto body_checksum = co_await google_crc32c_cooperatively(
    ...
    native_store(prefix->data() + 24, *body_checksum);
    comparison_crc header_checksum;
    header_checksum.extend(std::span<const char>{*prefix});
    native_store(prefix->data() + 28, header_checksum.value());
    if (
